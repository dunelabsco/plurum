...
import { getBlueprintServer } from "@/lib/api/blueprints-server";
import { BlueprintDetailContent } from "./blueprint-detail-content";
...
export default async function BlueprintDetailPage({ params }: PageProps) {
  ...
  try {
    // Fetch blueprint server-side - instant load!
    const blueprint = await getBlueprintServer(shortId);

    return <BlueprintDetailContent initialBlueprint={blueprint} shortId={shortId} />;
  } catch (error) {
    // Show error state
    return (
      <>
          ...
          <div className="mx-auto w-full max-w-5xl px-6 py-8">
            <div className="rounded-xl border border-dashed border-destructive/30 bg-destructive/5 p-12 text-center">
              ...
              <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
                This blueprint doesn&apos;t exist or has been removed from Plurum.
              </p>
              <Button asChild>
