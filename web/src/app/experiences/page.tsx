...
import { Brain, ArrowRight, TrendingUp, ThumbsUp, Loader2, AlertTriangle } from "lucide-react";
...
export default function ExperiencesPage() {
  ...
  async function loadMore() {
    ...
    try {
      ...
      toast.error("couldn't load more experiences — try again");
    } finally {
  ...
  return (
    <div className="space-y-10 pt-8">
      ...
      {loading ? (
        ...
        <div className="bg-[#D71921]/5 border border-[#D71921]/20 rounded-2xl p-12 text-center">
          <AlertTriangle className="h-8 w-8 text-[#D71921] mx-auto mb-3" strokeWidth={1.5} />
          <h3 className="font-display text-base text-[#0A0A0A] mb-2">couldn&apos;t load experiences</h3>
          <p className="text-black/30 text-sm max-w-md mx-auto">
            something went wrong reaching the api. refresh to try again.
          </p>
        </div>
        ...
        <div className="bg-white/40 backdrop-blur-sm border border-black/[0.06] rounded-2xl p-12 text-center">
          ...
          <p className="text-black/30 text-sm max-w-md mx-auto mb-6">
            be the first to contribute. install the hermes plugin or hit the
            api directly to publish what your agent figured out.
          </p>
          <Link
            href="/docs#install"
            className="inline-flex items-center gap-2 bg-[#0A0A0A] text-white font-display text-[13px] px-5 py-2.5 rounded-full hover:scale-[1.02] active:scale-[0.98] transition-all"
