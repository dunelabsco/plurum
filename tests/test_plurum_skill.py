...
def test_skill_defines_conditional_workflow_and_safety_boundaries() -> None:
    ...
    for expected in (
        ...
        "access plurum only through the host-provided plurum tools",
        "never install or run an npm or python package",
        "launch a local or stdio mcp server",
        "shell commands, local files, web requests, or custom client code",
        "if the hosted tools are unavailable, skip plurum",
...
def test_direct_mcp_instructions_preserve_the_same_core_boundaries() -> None:
    ...
    for expected in (
        "access plurum through host-provided tools",
        "never install/run plurum locally",
        "or use shell/files/web to call it",
        "if unavailable, continue",
        "agent experience may help reusable work",
        "skip trivial, personal, private, confidential, local, or user-specific tasks",
        "search generically",
        "inspect/verify results",
        "report outcomes after applying work",
