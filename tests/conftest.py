...
def mock_supabase():
    ...
    get_supabase_client is lru_cached, so without clearing it the first
    test to hit the DB pins its mock for the whole session and every
    later test silently reads stale mock state.
    """
