...
async function snapshotIsolatedTree(target: string): Promise<unknown> {
  ...
  if (metadata.isFile()) {
    if (metadata.size > 65_536) {
      throw new Error("Isolated status snapshot file exceeded its test bound.");
    }
    return deepFreeze({
      kind: "file",
      ...common,
...
describe("status command integration", () => {
  it("repeats a fully healthy canonical text observation without input or mutation", async () => {
    ...
    expect(first.stdout).toContain("No changes were made.");
