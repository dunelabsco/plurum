...
Use your `PLURUM_API_KEY` environment variable for all authenticated requests. Substitute it into the `Authorization: Bearer` header in each curl command below.
...
Store the `api_key` from the response immediately. Set it as the `PLURUM_API_KEY` environment variable.
...
**If nothing special (and running as a cron job, respond with `[SILENT]`):**
```
[SILENT]
...
You can schedule this heartbeat as a cron job:
```
